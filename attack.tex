We  will now describe an explicit attack against the NIPoPoW suffix proof construction under velvet fork. As already argued, since the protocol is implemented under velvet fork, any thorny block be accepted as valid. Taking advantage 
...
Assume that the adversary chooses to attack at some level $\mu_\mathcal{A}$. As shown in Figure \ref{fig:attack} 
she first generates a superblock $b'$ in her forked chain $\mathcal{C}_\mathcal{A}$ and a thorny block $a'$ in the honest chain $\mathcal{C}_B$ which points to $b'$. 
As argued earlier, block $a'$ will be accepted as valid in the honest chain $\mathcal{C}_B$ 
despite the invalid interlink pointers. After that, the adversary may mine on chain
$\mathcal{C}_\mathcal{A}$ or $\mathcal{C}_B$, or not mine at all. At some point she produces a thorny block $a$ 
in $\mathcal{C}_\mathcal{A}$ pointing to a block $b$ of $\mathcal{C}_B$. Because of the way blocks are generated
...
formulated which connects $\mathcal{C}_\mathcal{A}$ blocks $a$ and $b'$ and contains an arbitrarily
large part of the honest player's chain $\mathcal{C}_B$.

At this point the adversary will produce a suffix proof for chain $\mathcal{C}_\mathcal{A}$ containing
the subchain $\mathcal{C}\{ab\} \cup \mathcal{C}\{b:a'\} \cup \mathcal{C}\{a':b'\}$. Notice that following the
interlink pointers constructed in such a way, a light client perceives $\mathcal{C}\{ab\} \cup \mathcal{C}\{b:a'\} \cup \mathcal{C}\{a':b'\}$  as a valid chain.
...
\begin{figure}[h!]
	...
	\end{center}
	\caption{\textit{
		Chainsewing Attack. $\mathcal{C}_B$ represents the chain of an honest
	player. $\mathcal{C}_\mathcal{A}$ is an adversarial fork. Adversarially generated blocks are colored
