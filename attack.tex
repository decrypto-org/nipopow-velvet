...
different $0$-level chain. 
In fact, as the interlink vector contains multiple pointers, each pointer may
belong to a different fork.
The interlink pointing to arbitrary directions
resembles a thorny bush. These are shown in Figure~\ref{fig:thorny_blocks}.

\begin{figure}%
    \centering
    \subfloat[\centering ]{{\includegraphics[width=4cm]{figures/false_interlink.pdf} }}%
    \qquad
    \subfloat[\centering ]{{\includegraphics[width=4cm]{figures/thorny_block.pdf} }}%
    \caption{(a) A thorny block, colored black, in an honest party's chain, uses its interlink to point to a fork chain. (b) A thorny block appended to an honest party's chain. The dashed arrows are interlink pointers.}%
    \label{fig:thorny_blocks}%
\end{figure}
